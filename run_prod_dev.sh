...
if [ "$1" == "bg" ]; then
    ...
    if [ "$2" == "makemigrations" ]; then
        sudo docker-compose -f docker-compose.prod_dev.yml --env-file .env.prod exec django_prod python manage.py makemigrations
    elif [ "$2" == "migrate" ]; then
        sudo docker-compose -f docker-compose.prod_dev.yml --env-file .env.prod exec django_prod python manage.py migrate
    fi
else
